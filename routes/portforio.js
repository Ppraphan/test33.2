...
module.exports = function(app) {
  ...
  app.get('/researcher/:id', function(req, res) {
    ...
    con.query(sql1, function(err, results) {
      ...
      "SELECT * FROM project.university WHERE countryISOCode= '" + userCoutryID + "';"+
      "SELECT * FROM project.faculty WHERE uniID= '" + userUniID + "';"+
      "SELECT * FROM project.department WHERE facultyID= '" + userFacID + "';"+
      "SELECT * FROM project.sub_dpment WHERE Sub_Dpment_Parent= '" + userDpmentID + "';"+

      "SELECT * FROM project.users WHERE id= '" + userid + "';"+

      "SELECT * FROM project.project,project.RSProject,project.users,project.grants WHERE project.project.idproject = project.RSProject.idproject AND project.project.idGrant = project.grants.idGrants AND project.RSProject.idUser_RS = project.users.id AND project.users.id  = '" + userid + "' ;"+

      "SELECT * FROM project.project,project.LDProject,project.users,project.grants WHERE project.project.idproject = project.LDProject.idproject AND project.project.idGrant = project.grants.idGrants AND project.LDProject.idUser_LD = project.users.id AND project.users.id  = '" + userid + "' ;"+

      "SELECT * FROM project.portfolio WHERE userID = '" + userid + "';";
      ...
      con.query(sql2, function(err, results2) {
        ...
        res.render('pages/researcher', {
          ...
          dataRSProj:results2[6],
          dataLDProj:results2[7],
