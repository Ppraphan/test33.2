...
module.exports = function(app) {
  ...
  /*เปิดหน้าผลงานของฉัน*/
  app.get('/my-portforio',role.requireRole("admin","board","user"), function(req, res) {
  ...
  app.post('/my-portforio', function(req, res) {
    ...
    var portYear = req.body.portYear;
    ...
    if (portFile != '' && portFile != undefined) {
      ...
      if (filesize < pdfmaxsize) {
        ...
        var sql = "Insert into project.portfolio(userID,title,about,year,documentFile,categoryID) values('" + userid + "','" + portName + "','" + portAbout + "','" + portYear + "','" + userid + formatted + portFileName + "','" + portType + "')";
      ...
      sql1 = "Insert into project.portfolio(userID,title,about,year,documentFile,categoryID) values('" + userid + "','" + portName + "','" + portAbout + "','" + portYear + "','-','" + portType + "')";
      con.query(sql1);
  ...
  app.post('/my-portforio/update', function(req, res) {
    ...
    if (portFile != '' && portFile != undefined) {
      ...
      if (filesize < pdfmaxsize) {
        ...
        var sql = "UPDATE project.portfolio SET title = '" + portName + "', about = '" + portAbout + "' ,year = '" + portYear + "' , categoryID = '" + portType + "', documentFile = '" + portFileName + "' WHERE (idportfolio = '" + portID + "')";
        con.query(sql, function(err, result) {
          if (err) throw err;
          console.log("Insert Complete...");
        });
      ...
      var sql1 = "UPDATE project.portfolio SET title = '" + portName + "', about = '" + portAbout + "',year = '" + portYear + "' , categoryID = '" + portType + "' WHERE (idportfolio = '" + portID + "')";
      con.query(sql1);
      var query = con.query('SELECT * FROM project.portfolio', function(err, rows) {
        if (err)
          console.log("Error Selecting : %s ", err);
